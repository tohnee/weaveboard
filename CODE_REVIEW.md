...
## 2026-09 迭代九：Obsidian 暗色主题（引擎 v8）

- 用户对前两版浅色（v5 暖象牙、v6 冷蓝灰制图桌）均不满意，v8 整体切换为 **开发者暗色**（Linear/Geist 质感）：中性深底三层（画布 #0B0D11 / 面板 #101318-#131720 / 卡片 #161B22）、高对比墨白文字、靛蓝霓光主色 #5B63EA-#7C86F7（按钮/选中环/焦点/顶条）、珊瑚红 #E8554F 关键路径（绳索发光化：shadow pass 改为自身颜色 20% 光晕）、琥珀受阻、绿色完成。
- 组件质感升级：分段切换器激活态反白（浅底深字）、主按钮/工具激活带辉光、看板列语义底色（待评审靛蓝调/受阻琥珀调深色版）、toast 深底毛玻璃（首版反白 toast 被两轮视觉评审点名违和后修正）、画布加淡靛蓝氛围径向光。
- JS 侧 17 组颜色常量与种子数据 accent/edge 全部提亮适配暗底（如 task #7F9BBE→#82A7E8），新连线默认色同步。
- 验证：语法通过；四视图浏览器截图三轮视觉评审（看板 9/10、空间视图终版 9/10；修正反白 toast、绳索 alpha .62→.72、卡片边框提亮 #333D4E）；docs/index.html 与 skill 引擎资产同步 v8。
