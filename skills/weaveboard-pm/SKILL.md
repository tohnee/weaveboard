...
1. 用户给出进展（口头或 issue 列表）→ 更新对应任务的 `status` / `progress` / `actualHours`，受阻的加 blocker。
2. **基线纪律**：项目首次计划冻结时提醒用户在 WBS 概览点「设为基线」；此后每次更新关注"基线偏差" chip——顺延集中在关键路径上时必须在回报中指出。
3. **效率读数**：驾驶舱 SPI（EV/PV，进度绩效）与 CPI（EV/AC，按工时的投入效率）< 0.9 会在概览标红；向用户解释时给口径（EV=Σ进度%×预计工时）。
4. 数据落盘：编辑 JSON → 校验 → `inject_data.mjs` 重注入，或让用户在应用内编辑后导出 JSON 回传。
